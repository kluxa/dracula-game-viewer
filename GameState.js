...
class GameState {
	constructor() {
		...
		this.healths = [GameConstants.GAME_START_HUNTER_LIFE_POINTS,
						GameConstants.GAME_START_HUNTER_LIFE_POINTS,
						GameConstants.GAME_START_HUNTER_LIFE_POINTS,
						GameConstants.GAME_START_HUNTER_LIFE_POINTS,
						GameConstants.GAME_START_BLOOD_POINTS];
		this.locations = (new Array(5)).fill(LocationID.NOWHERE);
		this.moveHistory = [[], [], [], [], []];
	...
	backupState() {
		return {
			score: this.score,
			healths: this.healths.slice(),
			locations: this.locations.slice(),
			moveHistory: this.moveHistory.map(x => x.slice()),
			locationHistory: this.locationHistory.map(x => x.slice()),
			hunterViewOfDrac: this.hunterViewOfDrac.slice(),
			traps: this.traps.map(x => x[1]),
			vampireLocation: this.vampireLocation,
			researchCombo: this.researchCombo,
		};
	}
